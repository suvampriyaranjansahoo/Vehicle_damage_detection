...
python scripts/evaluate.py --model artifacts_oversampled/mobile_netv2_frozen.keras --csv artifacts/splits/test.csv
```
...
python scripts/fine_tune.py --model artifacts/mobile_netv2_frozen.keras --top-n 30 --epochs 5
```
...
For deployment, keep weights outside Git and set `MODEL_URL` to a hosted `.keras` artifact (Hugging Face Hub or object storage). The loader downloads once into a local cache and reuses it; `MODEL_PATH` remains the local fallback.
...
1. Host `model/car_damage_model.keras` (the served frozen model) on Hugging Face Hub or object storage.
2. Set `MODEL_URL` in the deployment environment.
